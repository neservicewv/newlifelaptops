...
export default function ReportsPage() {
  ...
  if (!data) return (
    <div className="p-8 text-[#8892b0] flex items-center gap-2">
      <BarChart2 size={16} className="animate-pulse" /> Loading reports...
    </div>
  );
  ...
  return (
    <div className="p-8 max-w-7xl">
      <h1 className="text-2xl font-black text-[#e6f1ff] font-mono uppercase tracking-wide mb-2">Reports & Analytics</h1>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div key={label} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-24 h-1.5 rounded-full bg-white/5 overflow-hidden">
                    <div className="h-full rounded-full" style={{width:`${data.totalDevices>0?((val||0)/data.totalDevices*100):0}%`,background:color}}/>
                  </div>
        ...
        <div className="card">
          <h2 className="text-sm font-semibold text-[#8892b0] uppercase tracking-wide mb-4">eBay Orders</h2>
          <div className="space-y-3">
            {[
              {label:'Total Orders',val:data.totalOrders,color:'#00a8ff'},
              {label:'Matched to Inventory',val:data.matchedOrders,color:'#10b981'},
              {label:'Unmatched',val:data.unmatchedOrders,color:'#f59e0b'},
            ].map(({label,val,color})=>(
              <div key={label} className="flex items-center justify-between">
                <span className="text-[#ccd6f6] text-sm">{label}</span>
                <span className="font-mono text-lg font-bold" style={{color}}>{val??0}</span>
              </div>
            ))}
          </div>
          <div className="mt-6 pt-4 border-t border-white/5">
            <h2 className="text-sm font-semibold text-[#8892b0] uppercase tracking-wide mb-3">Accessories</h2>
            <div className="flex items-center justify-between">
              <span className="text-[#ccd6f6] text-sm">Total Accessories</span>
